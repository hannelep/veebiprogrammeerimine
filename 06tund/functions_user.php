<?php
 ...
 function singIn($email, $password){
	...
	if($stmt->execute()){
	  ...
	  if($stmt->fetch()){
		...
		if(password_verify($password, $passwordFromDb)){
		  ...
		  $stmt->close();
		  $mysqli->close();
		  //siirdume teisele lehele, käsk header viib teisele lehele, location näitab kuhu lehele
		  header("Location: home.php");
		  //katkestame edasise tegevuse
		  exit();
		} else {
  ...
  function makeProfile($mydescription, $mybgcolor, $mytxtcolor){
    $notice = "";
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	...
	if($stmt->execute()){
		$notice = "Profiili muudatused õnnestusid!";
	} else {
		$notice = "Andmete salvestamisel tekkis viga!" .$stmt->error;
	}
